"""
run.py
Interactive chat loop over an existing agent and environment.
Creates a new session, shows tool-calling in real time, and deletes
the session on exit.

Prerequisites in .env (generated by setup_agent.py):
  AGENT_ID=agt_01...
...
def _run_turn(session_id: str, user_text: str) -> bool:
    """Open the stream, send the message, and process events until the agent goes idle.

    Returns False if the session was terminated (session.status_terminated),
    True in all other cases (including recoverable errors).
    """
    # Stream must be opened BEFORE sending: SSE only delivers events that occur
